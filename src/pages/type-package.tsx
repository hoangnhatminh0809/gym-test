...
const convertDurationToDays = (duration: string) => {
    // Split the duration string into parts: days, hours:minutes:seconds
    const [days, time] = duration.split(' ');

    // Convert the days part to a number (assuming it's valid)
    const numDays = parseInt(days, 10);

    // If the result is NaN (e.g., the string is not a valid number), return 0 or handle as needed
    if (isNaN(numDays)) {
        return 0; // Return 0 or handle error as required
    }
                                        ...
                                        <div className="grid gap-4 py-4">
                                            <div className="grid grid-cols-2 items-center gap-4">
                                                ...
                                                <Input
                                                    ...
                                                    type="number"
                                                    value={convertDurationToDays(editType.duration || '0 days')}
                                                    onChange={(e) => setEditType({ ...editType, duration: `${e.target.value} days` })}
                                                />
                                                <Label>Rate</Label>
                                                <Input
                                                    type="number"
                                ...
                                <TableBody>
                                    {types.map((item) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell>{convertDurationToDays(item.duration)} days</TableCell>
